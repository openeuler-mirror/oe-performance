<template>
  <el-table
    ...
    <el-table-column
      ...
      <template #default="scope">
        <div style="display: flex; align-items: center">
          <span :style="{ color: judgeLevelColor(scope.row.secretLevel) }">{{
      ...
      <template #default="scope">
        <div style="display: flex; align-items: center">
          <span
            class="is-dot"
            :style="{
              background: judgeProgressColor(scope.row.progress)
            }"></span>
    ...
    <el-table-column prop="detail" label="操作" fixed="right">
      <template #default="scope">
        <el-button link type="primary" @click="pushView(scope.row)"
          >查看</el-button
        >
...
const emit = defineEmits<{
  (e: 'pushView', obj: UserCenter.ApplicationDataItem): void // 设置自定义函数名称，参数类型与返回值类型
  // 可在此设置第二个自定义函数
}>()
const props = withDefaults(
  defineProps<{
    tableData: UserCenter.ApplicationData
  }>(),
  {
    tableData: () => []
  }
...
const state: UserCenter.ApplicationState = reactive({
  data: []
})
watchEffect(() => {
  state.data = tableData.value
})
...
function pushView(obj: UserCenter.ApplicationDataItem) {
  emit('pushView', obj)
}
</script>
